...
- **Honest framing (post-2026-04-28 reconciliation):** The 7-year backtest is on the canonical 58-pkg / 194-event manifest with **11 confirmed-exploited events**. Of those: NP+DI raw catches 5/11, NP+DI+DQ catches 8/11, hacker S+A catches 10/11, union catches 9/11. The remaining 1 (Tomcat HTTP PUT 2017) is absorbed by supplementary controls (floor-sweep + normal cadence — not WAFs; B-tier events don't earn WAF rule deployment). The 12-month sample has N=1 in-scope exploited CVE so "zero misses" framing on that window is vacuous and should not be reintroduced. **DO NOT reintroduce the older 6/13, 9/13, 10/13, 11/13 fractions** — those predate the manifest reconciliation.
...
- Check for new Claude-credited CVEs (search: "Carlini Claude CVE", "Mythos Preview credited", "MFSA Claude"). As of 2026-04-30, **six** have explicit credit: CVE-2026-4747 (FreeBSD, autonomous), CVE-2026-5194 (wolfSSL, Mythos-assisted), CVE-2026-5588 (Bouncy Castle, Carlini + Claude), CVE-2026-6746/6757/6758 (Firefox 150 — officially credited to Claude in MFSA 2026-30, 2026-04-21). If you find new ones, add them to the `claude_credited_cves` array and `claude_credited_notes` in config.json, and update the narrative in `glasswing.html` (Mythos page is where the credit narrative now lives — not in the walkthrough).
...
7. **Regenerate the canonical computed datasets** (in this order):
   ...
   - `python3 scripts/build_cve_reference.py` — re-unions every per-CVE artifact (KEV-scored events, hacker tiers, watch list, OSV NP+DI, di-reclassification, retro model run, daily analyst log). Writes `data/cve-reference.json` and patches the inline DATA blob in `docs/cve-reference.html` so the per-CVE audit page reflects the latest data. Run this AFTER you've updated config.json watch-list statuses (so any newly-confirmed entries pick up the kevDate) and AFTER any `data/*` artifact regen above.
   - `python3 scripts/refresh_post_apr1.py` — refreshes the dashboard "Live tracker (Apr 1, 2026 forward)" chart + per-event table. Hits OSV fresh for all 5 manifests (Spring, Node, Django, Netty, Real-world Java), filters to disclosure date >= 2026-04-01 + GHSA HIGH/CRITICAL, joins with the latest KEV snapshot + Metasploit + ExploitDB caches + `data/hacker-tiers.json`, applies the frozen DI CWE set, writes `data/post-apr1-per-framework.json`, then patches the embedded `POST_APR1` JS data block in `docs/dashboard.html`. Self-contained — no dependency on the periodicity caches that aren't refreshed daily. Network-bound (~150 OSV calls, 60-90s wall time). Re-run is safe and idempotent. Run AFTER `build_cve_reference.py` so the analyst's hacker-tier updates (if the analyst ran first) are picked up.
