...
- **Where the two methods agree, that's the validation. Where they disagree, that's informative.** On the 7-year backtest (canonical 58-pkg / 194-event manifest, 11 confirmed-exploited): NP+DI raw catches 5/11, NP+DI+DQ catches 8/11, hacker S+A catches 10/11, union catches 9/11. The remaining 1 (Tomcat HTTP PUT 2017) is absorbed by supplementary controls (floor-sweep + normal cadence, NOT WAFs — B-tier events don't earn WAF rules). **DO NOT reintroduce the older 6/13, 9/13, 10/13, 11/13 fractions** — those predate the 2026-04-28 manifest reconciliation.
...
- **Honest accounting:** Don't reintroduce "zero misses" framing for the 12-month window — that's vacuous (N=1). The 7-year backtest is where real scoring happens. On the canonical 11-event corpus, structure tests alone catch 8/11 (NP+DI+DQ); union catches 9/11; the 1 residual (Tomcat HTTP PUT 2017) absorbs into supplementary controls. Don't reintroduce the older 11/13 / 10/13 / 9/13 / 6/13 framings — they're pre-reconciliation.
...
2. **DI** (decision input). Y/N per the **frozen CWE rule** (frozen 2026-05-01, see `config.json:di_cwe_freeze`). The 31-CWE set: 22, 23, 35, 73, 74, 77, 78, 79, 89, 90, 91, 93, 94, 95, 96, 97, 113, 116, 287, 289, 306, 345, 444, 502, 611, 693, 776, 863, 917, 918, 1321. **DO NOT propose new CWE additions to this set, even if you encounter a confirmed-exploited CVE whose CWE isn't in it.** The freeze is intentional — it converts the "you fit the filter to the data" critique into forward-validation evidence. If the CVE's CWE isn't in the frozen set, mark DI=N and let the hacker S+A discriminator be the rescue path. If neither fires, log it as a genuine miss (that IS the public track record this freeze exists to build).
